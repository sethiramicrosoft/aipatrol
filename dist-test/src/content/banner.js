...
exports.showWarningBanner = showWarningBanner;
exports.removeWarningBanner = removeWarningBanner;
...
function showWarningBanner(result, onSendAnyway, onDismiss) {
    ...
    const accentColor = isBlock ? '#C0392B' : '#E67E22';
    const icon = isBlock ? '🚫' : '⚠️';
    const headline = isBlock
        ? 'GuardPrompt blocked sensitive data'
        : 'GuardPrompt detected sensitive data';
    ...
    <div style="display:flex; align-items:flex-start; gap:10px;">
      ...
      <div style="flex:1">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          <button id="gp-dismiss" style="background:#2d2d2d; color:#c0b8b0;">
            ${isBlock ? 'Close' : 'Dismiss'}
          </button>
          ${!isBlock ? `<button id="gp-send-anyway" style="background:#2d2d2d; color:#c0b8b0;">Send anyway</button>` : ''}
          <button id="gp-send-anyway-override" style="background:transparent; color:#666; font-size:12px; padding:7px 8px;">
            ${isBlock ? 'Send anyway ↗' : ''}
          </button>
        </div>
      </div>
  ...
  `;
    ...
    // Auto-dismiss after 12 seconds for WARN (not BLOCK)
    if (!isBlock) {
        setTimeout(() => removeWarningBanner(), 12000);
    }
}
